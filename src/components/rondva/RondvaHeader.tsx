...
// signed out). On phones the lockup and CTA shrink so both actions fit down to 320px.
export function RondvaHeader() {
  return (
    <header className="sticky top-0 z-40 border-b border-line/80 bg-paper/85 backdrop-blur supports-[backdrop-filter]:bg-paper/70">
      <div className="mx-auto flex max-w-[1120px] items-center justify-between gap-4 px-6 py-5 max-[359px]:gap-2 max-[359px]:px-4">
        <Link href="/" className="inline-flex shrink-0 items-center" aria-label="Rondva home">
          ...
          <img src="/rondva/logo.svg" alt="Rondva" width={137} height={39} className="h-auto w-[112px] min-w-24 max-[359px]:w-24 sm:w-[137px]" />
        </Link>
        <nav aria-label="Account" className="flex items-center gap-1 sm:gap-2">
          <a
            href={BRANDS.rondva.appUrl}
            className="inline-flex items-center whitespace-nowrap rounded-full px-3 py-2.5 text-sm font-semibold text-ink transition-colors hover:text-blue max-[359px]:px-2 sm:px-4"
          >
            Log in
          </a>
          <Link
